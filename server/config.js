export const UA = 'GHOSTCTF/1.0 (+https://example.local; passive CTF recon)';
...
export const limits = {
  ...
  /** Superfície HTML (href/forms) por host vivo — recon completo */
...
export function reconRateLimitConfig() {
  const max = Number(process.env.GHOSTCTF_RL_MAX ?? process.env.GHOSTRECON_RL_MAX ?? 12);
  const windowMs = Number(process.env.GHOSTCTF_RL_WINDOW_MS ?? process.env.GHOSTRECON_RL_WINDOW_MS ?? 60000);
