/**
 * Gera server/recon-pipeline.js a partir de GHOSTRECON/server/index.js
 * (helpers + runPipeline), removendo Shannon e PentestGPT.
 */
import fs from 'fs';
...
const src = path.join(root, 'GHOSTRECON', 'server', 'index.js');
const out = path.join(root, 'server', 'recon-pipeline.js');

const lines = fs.readFileSync(src, 'utf8').split('\n');

const importLines = [
  ...lines.slice(6, 85), // 7-85 (skip express, randomBytes, fileURLToPath duplicate handled below)
  ...lines.slice(93, 101), // 94-101 mitre/owasp
];
...
const helpers = lines.slice(198, 313).join('\n'); // aiAutoReports… buildPipelineExportPayloadForAi

let body = lines.slice(314, 2098).join('\n'); // runPipeline … end
